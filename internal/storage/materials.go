...
func (r *MaterialsStorage) GetAllBlobData(ctx context.Context, userID int) ([]domain.BlobData, error) {
	getTextDataStmt, err := r.db.PrepareContext(ctx, "SELECT id,data,metadata FROM blob_data WHERE user_id=$1;")
	if err != nil {
		return nil, &StatementPSQLError{Err: err}
	}
	defer getTextDataStmt.Close()
...
func (r *MaterialsStorage) UpdateBlobDataByID(ctx context.Context, userID int, data domain.BlobData) error {
	updateTextDataStmt, err := r.db.PrepareContext(ctx, "UPDATE blob_data SET data = $1, metadata = $2 WHERE user_id = $3 and id = $4;")
	if err != nil {
		return &StatementPSQLError{Err: err}
	}
	defer updateTextDataStmt.Close()
...
func (r *MaterialsStorage) CreateNewBlobData(ctx context.Context, userID int, data domain.BlobData) error {
	crUserStmt, err := r.db.PrepareContext(ctx, "INSERT INTO blob_data (user_id, data, metadata) VALUES ($1, $2, $3);")
	if err != nil {
		return &StatementPSQLError{Err: err}
	}
	defer crUserStmt.Close()
